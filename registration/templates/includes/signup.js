	...
	function set_availability_status(is_available, subdomain, validation_msg) {
		...
		if(is_available) {
			...
			page.find('.availability-status span').html(`${subdomain}.punia.online is available!`);
		} else {
			// not available state
			...
			page.find('.availability-status span').html(`${subdomain}.punia.online is already taken.`);
		}
	}
	...
	page.find('.btn-request').off('click').on('click', function() {
		...
		$btn.prop("disabled", true).html("Mengirim data...");
		...
		frappe.call({
			method: 'punia_website.api.signup',
			args: args,
			type: 'POST',
			btn: $btn,
			callback: function(r) {
	...
	function check_if_available(subdomain, callback) {
		setTimeout(function() {
			frappe.call({
				method: 'punia_website.api.check_subdomain_availability',
	...
	if (!query_params.plan) {
		...
		// for mobile app signup, hide header and footer
		$("header,footer").addClass("hidden");
	}

	page.find(".plan-message").text("Paket " + query_params.plan + " / " + query_params.period + " Bulan" + " / Rp " + query_params.price );
